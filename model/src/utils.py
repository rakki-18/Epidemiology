...
class SIR:
    ...
    def create_contact_graph(self):
        ...
        while index < self.df.shape[0]:
            id1 = self.df['p1'][index]
            id2 = self.df['p2'][index]
    ...
    def visualize_graph(self, G, vaccinated, days, vaccination_day):
        ...
        nx.draw_networkx(G, pos=pos, nodelist=list(
            self.susceptible), node_size=1800, node_color='dodgerblue', font_size=17)
        nx.draw_networkx(G, pos=pos, nodelist=list(self.infected),
                         node_size=1800, node_color='orange', font_size=17)
        nx.draw_networkx(G, pos=pos, nodelist=list(
            self.recovered), node_size=1800, node_color='limegreen', font_size=17)
        nx.draw_networkx(G, pos=pos, nodelist=list(self.deceased),
                         node_size=1800, node_color='orangered', font_size=17)
        if (days >= vaccination_day):
            nx.draw_networkx(G, pos=pos, nodelist=list(
                vaccinated), node_size=1800, node_color='yellow', font_size=17)
        ...
        plt.legend(handles=[S_blue, I_orange, R_green,
                   D_red, V_yellow], prop={"size": 20})
        plt.show()
...
def simulate(model, timestamps, vaccinated, vaccination_day):
    ...
    while total_count < model.df.shape[0]:
        ...
        while count < timestamps and total_count < model.df.shape[0]:
            person1 = int(model.df['p1'][total_count])
            person2 = int(model.df['p2'][total_count])
            ...
            if(model.df['timestamp'][total_count] != previous_timestamp):
                previous_timestamp = model.df['timestamp'][total_count]
                count = count + 1
